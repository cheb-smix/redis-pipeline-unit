...
use Yii;
use console\components\websocket\SmixWebSocketServer;

class WSDaemon extends SmixWebSocketServer
{
    // protected $DBPool = [];
    // protected $DBPoolLength = 0;
    ...
    public function __construct(array $config = [])
    {
        ...
        //     $config["db"]["connections"] = 1;
        // }

        // try {
        //     for ($i = 0; $i < $config["db"]["poollength"]; $i++) {
        //         $this->DBPool[] = new SafeMySQL($config["db"]);
        //     }
        // } catch (\Exception $e) {
        //     $this->printer("MySQL connection error: " . $e->getMessage());
        // }

        // $this->DBPoolLength = count($this->DBPool);

        // if ($this->DBPoolLength < $config["db"]["poollength"]) {
        //     $this->printer("MySQL connections length is not equals to specified: [" . $this->DBPoolLength . "/" . $config["db"]["poollength"] . "]");
        // } else {
        //     $this->printer("MySQL connected successfully! [" . $this->DBPoolLength . "/" . $config["db"]["poollength"] . "]");
        // }

        // if (count($this->DBPool) == 0) {
        //     $this->printer("MySQL. No connections. Exiting... [" . $this->DBPoolLength . "/" . $config["db"]["poollength"] . "]", false, true);
        // }

        $this->config = $config;

        foreach ($this->config["cacheInstances"] as $dbnum => $instanceName) {
            $this->pipeline[$dbnum] = [];
        }

        $this->pipewidth = $this->config["pipewidth"];
        $this->pipelineMinClients = $this->config["pipelineMinClients"];

        parent::__construct(
            $this->config["socket"]["protocol"] . "://" . $this->config["socket"]["ip"] . ":" . $this->config["socket"]["port"],
            $this->config["socket"]["monitorAllowedIpPool"]
        );
    }
    ...
    protected function onLoop()
    {
        ...
        foreach ($this->config["cacheInstances"] as $dbnum => $instanceName) {
            if (!$this->pipeline[$dbnum]) {
                continue;
            }
            if ($connectionsCnt < $this->pipelineMinClients || count($this->pipeline[$dbnum]) >= $this->pipewidth) {
                $this->executePipeline($dbnum);
                continue;
            }
            if (array_sum(array_column($this->pipeline[$dbnum], "count")) >= $this->pipewidth) {
                $this->executePipeline($dbnum);
                continue;
            }
        }
    ...
    protected function executePipeline($dbnum)
    {
        ...
        $results = Yii::$app->{$this->config["cacheInstances"][$dbnum]}->pipeline($cmds);
        ...
        foreach ($this->pipeline[$dbnum] as $requestHash => $request) {
            foreach ($request["clients"] as $i => $cid) {
                $this->outputData($cid, "OK", $results[$reqIndex]);
    ...
    protected function onMessage($cid, $request)
    {
        ...
        if (!in_array($cmd, $this->redisCommands)) {
            $this->outputData($cid, "OK", false);
            return;
        }
        if ($cmd == "SELECT") {
    ...
    protected function getMonitoring($cid, $data = []) // фича для MySQL статы, пока не нужна
    {
        // if ($data = $this->socketStatistics($cid)) {
        //     $data["MemUsed"] = memory_get_usage();
        //     $rnd = rand(0, $this->DBPoolLength - 1);

        //     return array_merge($data, $this->DBPool[$rnd]->getIndCol("Variable_name", "SHOW SESSION STATUS WHERE Variable_name IN ('Connections', 'Bytes_received', 'Bytes_sent', 'Queries', 'Threads_connected', 'Threads_running')"));
        // }
